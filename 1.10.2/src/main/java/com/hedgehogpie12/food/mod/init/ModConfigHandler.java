...
public class ModConfigHandler {
	...
	public static void init() {
		...
		/**
		 * NOT IN 1.10.2 Reference.DONKEY_DROP_AMOUNT = CONFIG.getInt("Donkey
		 * Drop Amount", DEF_CAT, 2, 0, 2147483647, "Increase or Decrease drop
		 * *amount* not RATE!(Note: Remember high numbers can be more laggy as
		 * it creates more entities!)"); Reference.MULE_DROP_AMOUNT =
		 * CONFIG.getInt("Mule Drop Amount", DEF_CAT, 2, 0, 2147483647,
		 * "Increase or Decrease drop *amount* not RATE!(Note: Remember high
		 * numbers can be more laggy as it creates more entities!)");
		 * 
		 * Reference.PARROT_DROP_AMOUNT = CONFIG.getInt("Parrot Drop Amount",
		 * DEF_CAT, 2, 0, 2147483647, "Increase or Decrease drop *amount* not
		 * RATE!(Note: Remember high numbers can be more laggy as it creates
		 * more entities!)"); Reference.LLAMA_DROP_AMOUNT = CONFIG.getInt("Llama
		 * Drop Amount", DEF_CAT, 2, 0, 2147483647, "Increase or Decrease drop
		 * *amount* not RATE!(Note: Remember high numbers can be more laggy as
